...
{
    Log::Message("[Vulkan]\tCreating Vulkan Instance.\n");
    ...
    Log::Success("[Vulkan]\tVkInstance created at [0x%08x]!\n", instance);
    return instance;
}
...
{
    Log::Message("[Vulkan]\tCreating Logical Device.\n");
    ...
    Log::Success("[Vulkan]\tLogical Device Created! [0x%08x]\n", device);
    return device;
}
...
{
    Log::Message("[Vulkan]\tSelecting a Physical Device.\n");
    ...
    Log::Success("[Vulkan]\tPhysical Device %d Selected!\n", deviceIdx);
    return device;
}
...
{
    Log::Message("[Vulkan]\tCreating Window Surface\n");
...
#endif
    Log::Success("[Vulkan]\tWindow Surface Created at [0x%08x]\n", surface);
    return surface;
}
...
{
    Log::Message("[Vulkan]\tDestroying Window Surface");
...
{
    Log::Message("[Vulkan]\tValidating Device extension %s.\n", extension);
    ...
    for (uint32_t i = 0; i < numDeviceSupportedExtensions; i++)
    {
        if (strcmp(instanceSupportedExtensions[i].extensionName, extension) == 0) {
            Log::Success("[Vulkan]\tExtension %s is supported by the current Physical Device!\n", extension);
            supported = true;
            goto ExtensionFound;
        }
    ...
    Log::Failure("[Vulkan]\tExtension %s was not supported by the current Physical Device.\n", extension);
...
bool Helpers::ValidateArrayDeviceExtensionSupport(const VkPhysicalDevice& device, const char** const pExtensions, const uint32_t numExtensions)
{
    ...
    if (pExtensions == nullptr || numExtensions == 0) {
        Log::Warning("[Vulkan]\tValidateArrayDeviceExtensionSupport() was called, but pExtensions was [0x%08x], and numExtensions was %d!\nDefaulting to False\n", pExtensions, numExtensions);
        return false;
    }
...
{
    Log::Message("[Vulkan]\tValidating extension %s.\n", extension);
    ...
    {
        if (strcmp(instanceSupportedExtensions[i].extensionName, extension) == 0) {
            Log::Success("[Vulkan]\tExtension %s is supported by the current Instance!\n", extension);
            supported = true;
            goto ExtensionFound;
        }
    ...
    //Report if we didn't find the extension. 
    Log::Failure("[Vulkan]\tExtension %s was not supported by the current Instance.\n", extension);
...
bool Helpers::ValidateArrayInstanceExtensionSupport(const char** const pExtensions, const uint32_t numExtensions)
{
    ...
    if (pExtensions == nullptr || numExtensions == 0) {
        Log::Warning("[Vulkan]\tValidateArrayInstanceExtensionSupport() was called, but pExtensions was [0x%08x], and numExtensions was %d!\nDefaulting to False\n", pExtensions, numExtensions);
        return false;
    }
...
{
    Log::Message("[Vulkan]\tValidating layer %s.\n", layer);
    ...
    for (uint32_t i = 0; i < numInstanceSupportedLayers; i++)
    {
        if (strcmp(instanceSupportedLayers[i].layerName, layer) == 0) {
            Log::Success("[Vulkan]\tLayer %s is supported by the current Instance!\n", layer);
            supported = true;
            goto LayerFound;
        }
    ...
    Log::Failure("[Vulkan]\tLayer %s was not supported by the current Instance.\n", layer);
...
bool Helpers::ValidateArrayInstanceLayerSupport(const char** const pLayers, const uint32_t numLayers)
{
    ...
    if (pLayers == nullptr || numLayers == 0) {
        Log::Warning("[Vulkan]\tValidateArrayInstanceLayerSupport() was called, but pLayers was [0x%08x], and numLayers was %d!\nDefaulting to False\n", pLayers, numLayers);
        return false;
    }
...
{
    Log::Message("[Vulkan]\tCreating a Debug Logger.\n");
    ...
    Log::Success("[Vulkan]\tDebug Logger created at [0x%08x] (allocator [0x%08x])!\n", &debugLogger, pAllocator);
...
{
    Log::Message("[Vulkan]\tDestroying Debug Messenger [0x%08x].\n", &messenger);
...
{
    Log::Message("[Vulkan]\tDestroying Vulkan Instance at [0x%08x] (allocator [0x%08x])\n", instance, pAllocator);
    vkDestroyInstance(instance, pAllocator);
}

void Helpers::DestroyLogicalDevice(VkDevice& device, const VkAllocationCallbacks* pAllocator)
{
    Log::Message("[Vulkan]\tDestroying Logical Device [0x%08x]", device);
...
{
    Log::Message("[Vulkan]\tCreating Swapchain\n");
...
VkBuffer Helpers::CreateBuffer(const VkDevice& device, size_t size, VkBufferCreateFlags usageFlags, VkSharingMode sharingMode, const VkAllocationCallbacks* pAllocator){
    Log::Message("[Vulkan]\tCreating Buffer of size %d:\n\tUsage Flags: 0x%x\n\tSharing Mode: 0x%x\n", size, usageFlags, (uint32_t)sharingMode);
